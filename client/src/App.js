...
const useConnected = (cookies) => {
  ...
  const parsedCookie = (cookies, index) => cookies.split(";").map((cookie) => cookie.split("=")[1]);
...
const Layout = () => {
  ...
  return (
    <Router>
      ...
      <Switch>
        <Route path="/" exact component={() => <Home connected={connexion} currentUser={currentUser} />} />
        <Route path="/login" exact component={Login} />
        <Route path="/register" exact component={Register} />

        <Route path="/newpost" exact component={() => <NewPost token={token} currentUser={currentUser} />} />
        <Route path="/profile" exact component={() => <Profile currentUser={currentUser} />} />
      </Switch>
    </Router>
  );
};
